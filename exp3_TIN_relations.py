...
dataset_list = ['CF_amt']
# dataset_list = ['CF', 'CF_amt', 'd_Duck_Identification', 'd_jn_product', 'd_sentiment', 'MS', 's4_Dog_data',
#                 's4_Face_Sentiment_Identification', 'SP', 'SP_amt', 'ZenCrowd_all',
#                 'ZenCrowd_in', 'ZenCrowd_us']
method_name = 'TIN'
...
for i in range(len(dataset_list)):
    ...
    kwds = {'task_community_file': task_community_file}
    ...
    for beta_1 in np.linspace(0, 1, 21):
        ...
        for beta_2 in np.linspace(0, 1 - beta_1, int(round((1.05 - beta_1) / 0.05))):
            ...
            results[dataset][beta_1][beta_2] = pool.apply_async(get_method_accuracy_in_data, args=(
                method_name, datafile_validation, truth_file, beta_1, beta_2, beta_3),
                                                                kwds={})
results_test = {}
for i in range(len(dataset_list)):
    dataset = dataset_list[i]
    ...
    results_test[dataset] = pool.apply_async(get_method_accuracy_in_data, args=(
        method_name, datafile_test, truth_file, opt_beta_1, opt_beta_2, opt_beta_3),
                                             kwds={})
for i in range(len(dataset_list)):
    dataset = dataset_list[i]
